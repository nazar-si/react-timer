...
const indicatorStyle = "absolute h-1 w-1/3 left-1/2 -translate-x-1/2 bottom-0 rounded-t-md transition-all duration-300 opacity-0";
...
export default function ModeSwitch() {
    ...
    <Button className="px-1 !py-0 flex items-center relative group flex-1">
        ...
        {Object.entries(modes).map(([key, value], i) => <div key={key} className={modeStyle} onMouseOver={()=>setHovered(i)} onClick={()=>setMode(key as keyof typeof modes)}>
            {value.name}
            <div className={indicatorStyle + ` ${value.color} ${mode === key?indicatorActiveStyle:""}`}></div>
        </div>)}
    </Button>
  )
}