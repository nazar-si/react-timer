...
describe('Modal', () => {
    ...
    it('should close when clicking outside', ()=>{
        ...
        expect(dialog.classList.contains('opacity-0')).toBe(false);
        act(()=>{dialog.click()});
        expect(screen.queryByRole('dialog')?.classList.contains('opacity-0')).toBe(true);
    });
    it('should not close when clicking inside', ()=>{
        const MockContainer = ()=>{
            const [show, setShow] = React.useState(true);
            return <Modal show={show} setShow={setShow}><button aria-label='do nothing'>Test</button></Modal>
        }
        render(<MockContainer/>);
        const dialog = screen.getByRole('dialog');
        ...
        expect(dialog.classList.contains('opacity-0')).toBe(false);
        act(()=>{button.click()});
        expect(screen.queryByRole('dialog')?.classList.contains('opacity-0')).toBe(false);
    });
});