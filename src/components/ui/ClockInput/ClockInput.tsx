...
const NumberInput = Object.assign(forwardRef<HTMLSpanElement, NumberInputProps>(({value, setValue, ...props}, ref)=> {
        ...
        {Array(props.max)
          ...
          .map((_, i) => {
            return <div 
              ...
              className={"absolute -translate-y-1/2 transition-all duration-200 flex items-center justify-center"}
              style={{
                top: `calc(50% - ${(i - Math.floor(props.max / 2)) * 40}px)`
              }}
              >
              <div className='bg-transparent items-center text-center cursor-pointer caret-transparent'>
                {(i + value + Math.floor(props.max / 2) + 2)%(props.max + 1)}
              </div>
            </div>;
          })}
      </div>
...
export default function ClockInput({value, setValue}: Props) {
  const refs = Array(4).fill(0).map(()=>React.useRef<HTMLSpanElement>(null));
  const Focus = (i: number)=>{
    refs[i].current?.focus();
  }
  const valueState = Array(4).fill(0).map(()=>useState(0));
  useEffect(()=>{
  ...
  const useSetValue = (n: number) => ((v: number | ((v:number)=>number)) => {
    let values = valueState.map(i=>i[0]);
